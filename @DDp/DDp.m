% Mode: 1 = Perpetual, default
% 2 = Discreet.
% If discreet, need to specify specific vectors for each model
% If perp, generate stochastically on each step

classdef DDp
    properties
        mode = 1 % Perpetual, 2 = Discreet
        model % Model to use
        func % Eq for model
        ...
        its % Total iterations to keep
        aNoise % Vector or Function to generate acc noise
        aMu % Mean of accumulator noise
        aSig % std of accumulator noise
        aLam % Acc auto-regressive parameter
        sNoise1 % Function or vector to generate noise1, string
        sNoise2 % Function or vector to generate noise2, string
        sMu % Mean of sensory noise
        sSig % std of sensory noise
        initialParams % Save specified params
        delta1 % Vector or Function to generate delta1
        delta2 % Vector ot Function to generate delta2
    ...
    methods
        function obj = DDp(params, stim) % Initialise
            obj.initialParams = params;
            
            % Has mode been specified?
        ...
        function obj = preallocate(obj, params, stim)
            % Prealloate accumulator noise
            % Generate acc noise
            if ~isnan(obj.aMu)
        ...
        function obj = setModel(obj, params, stim) % Set the model
            ...
            % And redo preallocation if needed
            switch obj.mode
                case 1
                case 2
                    obj = obj.preallocate(params, stim);
            end
        ...
        function obj = iterate(obj, steps) % Run set no. of its
            ...
            for i = st:ed
                switch obj.mode
                    case 1
                        % First check on buffer
                        if obj.it == obj.its
                            % If filled, remove first, replace last
                            obj.it = obj.its;
                            obj.output(2:end) = obj.output;
                        end
                        switch obj.model
                            % Commented: Not yet converted to perp
                            case 'ARGuass'
                                % obj.output(i) = ...
                                %    obj.aLam*obj.output(i-1) + obj.aNoise(i); % A
                                obj.output(i) = obj.aLam*obj.output(i-1) + obj.aMu + randn * obj.aSig;
                                obj.delta1(i) = NaN;
                                obj.delta2(i) = NaN;
                            case 'Gareth1D';
                                % obj.output(i) = ...
                ...
                switch obj.decBoundMode
                    ...
                    case 'Linear'
                        obj.outputDecBoud(i) = obj.decFuncParams(1) * obj.its +  obj.decFuncParams(2);
                    ...
                    case 'Custom'
                        % Fuunction defined in obj.decFunc
                ...
                % If in perpetual mode, check buffer and shift if nes
                switch obj.mode
                    case 1
                        if obj.it == obj.its
                            obj.output = obj.output(2:end);
                            obj.outputDecBound = obj.outputDecBound(2:end);
        ...
        function [obj, outputVal] = getNext(obj)
            obj = obj.run(1);
            
            outputVal = obj.output(obj.it);
        end
        
        function obj = run(varargin) % Run all remaining steps
            obj = varargin{1};
